...
export default function Page({ subHeaderName = "Sandbox" }) {
  ...
  const step = ({ text = '', selected = false, done = false, onClick = () => {} }) => <div style={{
    ...
    className={`border border-[${selected ? '#407BFF' : '#EEEDF0'}] border-solid dark:border-[${selected ? '#407BFF' : '#050505'}]`}
    onClick={onClick}
  >
          ...
          {reviewOpts.map(({ name, inputName, description, selected }, i, a) => <>
            <div className="items-top flex space-x-2">
              ...
              <div className="grid gap-1.5 leading-none" style={{ marginLeft: 15 }}>
                <Label htmlFor="email" className="font-bold text-md mb-[10px]">{name}</Label>
  ...
  const surveyQuestions = [
    ...
    },
    {
      ...
      answerComponent: (disableForm: boolean) => <CardContent className="flex flex-col flex-1 pb-[40px] px-[50px]">
        <div className="flex flex-row flex-wrap mb-[15px]">
        ...
        >
          ...
          >
            <div style={{ transition, position: 'relative', top: 0, display: 'flex', flex: 1, flexDirection: 'column', maxWidth: 540, padding: '0px 25px' }}>
              <div className="dark:text-white text-black" style={{ fontWeight: 'bold', fontSize: 24, margin: '52px 0px 25px 0px' }}>{name}</div>
